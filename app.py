...
@app.route('/ruc-info', methods=['POST'])
def ruc_info():
    # Solicitud para consultar la información del RUC
    ruc = request.json.get('ruc')  # Asegúrate de que estamos usando JSON
    token = request.headers.get('Authorization').split(" ")[1]  # Obtener el token del encabezado

    if not ruc or not token:
        return jsonify({"error": "RUC o token faltante"}), 400
...
@app.route('/ruc-info-view', methods=['GET'])
def ruc_info_view():
    # Solicitud para mostrar la información del RUC desde la URL
    ruc = request.args.get('ruc')

    if not ruc:
        return render_template('ruc-info.html', error="RUC faltante")
